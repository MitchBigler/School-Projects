...
namespace CS3260_Lab12_KH
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Query 0 – Find the values in odds that are divisible (no remainder) by three (3)
            //           and write these values to the Console.
            Console.WriteLine("---- QUERY 0 ----");

            IEnumerable<int> Query0 =
                from val in odds
                where val % 3 == 0
                select val;

            foreach (int val in Query0) // output query result
            { 
                Console.WriteLine(val);
            }


            // Query 1 – Find values in data where the ID is greater than 100 and less than 500
            //           and the Name contains a ‘y’ or Job ends with ‘e’ or Job contains ‘e’ or
            //           Job equals “Student” or Job equals “Programmer”. Write these values to the Console.
            Console.WriteLine("\n---- QUERY 1 ----");

            var Query1 =
                from record in data
                where record.ID > 100 && record.ID < 500 && 
                    (
                    record.Name.Contains("y") || record.Job.EndsWith("e") || record.Job.Contains("e") || record.Job == "Student" || record.Job == "Programmer"
                    )
                select record;

            foreach (var r in Query1) // output query result
            { 
                Console.WriteLine($"{r.ID} {r.Name} {r.Job}");
            }

            // Query 2 – Find values in productList where the ProductID is greater than 10 and
            //           less than 66, order in descending order by UnitsInStock and write the
            //           ProductName and UnitsInStock to the Console.
            Console.WriteLine("\n---- QUERY 2 ----");
            ...
            foreach (var product in Query2)  // output query result
            {
                Console.WriteLine($"{product.ProductName}: {product.UnitsInStock}");
            }

        Console.ReadLine(); // wait for input to close window
